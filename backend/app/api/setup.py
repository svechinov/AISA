...
class LLMRowIn(BaseModel):
    provider: Literal["claude", "gemini", "openai", "perplexity", "grok"]
    api_key: str = ""
...
class SetupBootstrapIn(BaseModel):
    ...
    llm_rows: list[LLMRowIn] = Field(..., min_length=len(VALID_LLM), max_length=len(VALID_LLM))
    ...
    def llm_rows_unique_providers(cls, rows: list[LLMRowIn]) -> list[LLMRowIn]:
        ...
        if set(ids) != VALID_LLM:
            raise ValueError(f"Must include all {len(VALID_LLM)} LLM providers once")
        return rows
