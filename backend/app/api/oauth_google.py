...
class GmailOAuthStartIn(BaseModel):
    ...
    mailbox_email: str | None = Field(
        default=None,
        description=(
            "Which mailbox to connect (B-071 stage B), e.g. stepan@alexstaff.agency. "
            "Omit to connect/refresh the default global mailbox (Alexey), same as before."
        ),
    )
...
def gmail_oauth_start(body: GmailOAuthStartIn) -> dict[str, str]:
    ...
    mailbox_email = (body.mailbox_email or "").strip() or None
...
def gmail_oauth_callback(code: str | None = None, state: str | None = None) -> RedirectResponse:
    ...
    mailbox_email = str(data.get("mailbox_email") or "").strip() or None
    if not public_origin or not redirect_uri:
        raise HTTPException(status_code=400, detail="Invalid OAuth state payload")

    try:
        verify_gmail_roundtrip_with_code(code, redirect_uri, mailbox_email)
