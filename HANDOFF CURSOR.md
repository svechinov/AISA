...
**Human UI: загрузка run, workspace-lite, контакты/черновики, гонки запросов:** **`HANDOFF.md` §15** — обязательно перед правками **`frontend/src/pages/AiBizOsHumanUI.jsx`**. Диаграмма запросов в §14.3 частично устарела; актуальная схема — **§15.3**.

**Операторский UI и Gmail sync (релиз 2.4.0 в тексте):** **`HANDOFF.md` §13**; исторический снимок 2.1.4 — **§12**. Актуальный номер фронта всегда смотреть в **`frontend/package.json`** (сейчас **2.7.2**).
